...
def main():
	...
	for file in invoices_list: 
		...
		if os.path.isfile (check_history):
			overwriten = raw_input("File:"+file+" founded in history folder, Do you want to overwrite and reprocess this file again? [yes/no]:");
			print ("\n");
		...
		if result == 0: 
			...
			if result == 1: 
				...
				print ("converted result: format is not same as in spec");
				os.rename(parse_folder+"\\\\working\\\\"+file,parse_folder+"\\\\manual\\\\"+file);
			try:
